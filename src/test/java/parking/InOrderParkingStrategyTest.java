...
public class InOrderParkingStrategyTest {
    ...
    public void testCreateReceipt_givenACarAndAParkingLog_thenGiveAReceiptWithCarNameAndParkingLotName() {
	    ...
	    * With using Mockito to mock the input parameter */
        ...
        when(myCar.getName()).thenReturn("myCar");

        ParkingLot mParkingLot = mock(ParkingLot.class);
        when(mParkingLot.getName()).thenReturn("Parking Lot A");

        InOrderParkingStrategy inOrderParkingStrategy = new InOrderParkingStrategy();
        Receipt receipt = inOrderParkingStrategy.createReceipt(mParkingLot, myCar);

        verify(myCar, times(1)).getName();
    ...
    public void testCreateNoSpaceReceipt_givenACar_thenGiveANoSpaceReceipt() {
        ...
        when(myCar.getName()).thenReturn("myCar");
        InOrderParkingStrategy inOrderParkingStrategy = new InOrderParkingStrategy();
        Receipt receipt = inOrderParkingStrategy.createNoSpaceReceipt(myCar);
        verify(myCar, times(1)).getName();
        assertEquals("myCar", receipt.getCarName());
